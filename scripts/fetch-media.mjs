...
const localPath = (u) => {
  ...
  return url.host === 'kristenpardue.com'
    ? path.join(PUB, rel)
 ...
 */
const EXTRA = [
  'https://kristenpardue.com/wp-content/plugins/elementor/assets/css/conditionals/dialog.min.css',
];
...
const workers = Array.from({ length: 8 }, async () => {
  while (queue.length) {
    ...
    try {
      const res = await fetch(u, { headers: { 'user-agent': UA, referer: 'https://kristenpardue.com/' } });
